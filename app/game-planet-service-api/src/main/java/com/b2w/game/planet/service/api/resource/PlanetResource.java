...
/**
 * Since the spec is brief I assume it's OK to choose the protocol:
 * I favor simplicity and security (not informing the client concerning 
 * back-end errors/availability that's another API).
 * 
 * @author msa
 *
 */
...
public class PlanetResource {
    ...
    private Response response(PlanetWFilm planet) {
        return planet == null ? Response.status(NOT_FOUND).build() : Response.ok(planet).build();
    }

    @GET
    @Produces({ MediaType.APPLICATION_JSON })
    public Response lista(@QueryParam("nome") @Size(min = 1, max = 50) @Pattern(regexp = Planet.NM_PAT) String nome) {
        if (nome != null) {
            return response(dao.readw(nome.trim()));
        }
    ...
    public Response novo(@NotNull @Valid Planet planet) {
        if (dao.create(planet)) {
            ...
            try {
                ...
                if (chk != null) resource = new URI(base.getBaseUri() + "/" + chk.getId());
